...
// hyperlink
SpawPGcore.hyperlinkClick = function(editor, tbi, sender,paths)
{
  if (tbi.is_enabled)
  {
    var a = editor.getSelectedElementByTagName("a");
    editor.stripAbsoluteUrl(a);
    SpawEngine.openDialog('core', 'hyperlink', editor, a, '', 'SpawPGcore.hyperlinkClickCallback', tbi, sender,paths);
  }
}
...
// image
SpawPGcore.imageClick = function(editor, tbi, sender,paths)
{
  if (tbi.is_enabled)
  {
    SpawEngine.openDialog('spawfm', 'spawfm', editor, '', 'type=images', 'SpawPGcore.imageClickCallback', null, null,paths);  
  }
}
...
SpawPGcore.imagePropClick = function(editor, tbi, sender)
{
  if (tbi.is_enabled)
  {
    ...
    if (i)
    {
      ...
      SpawEngine.openDialog('core', 'image_prop', editor, i, '', '', tbi, sender,paths);
    }
    else
    {
      // new image
      SpawEngine.openDialog('core', 'image_prop', editor, i, '', 'SpawPGcore.imagePropClickCallback', tbi, sender,paths);
    }
  }
}
...
SpawPGcore.flashPropClick = function(editor, tbi, sender)
{
  if (tbi.is_enabled)
  {
    ...
    if (i)
    {
      // editing
      editor.stripAbsoluteUrl(i);
      SpawEngine.openDialog('core', 'flash_prop', editor, i, '', '', tbi, sender,paths);
    }
    else
    {
      // new flash
      SpawEngine.openDialog('core', 'flash_prop', editor, i, '', 'SpawPGcore.flashPropClickCallback', tbi, sender,paths);
    }
  }
}
