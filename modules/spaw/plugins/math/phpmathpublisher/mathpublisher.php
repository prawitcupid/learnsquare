...
$dirfonts=$_SERVER["DOCUMENT_ROOT"].$_SERVER["PHP_SELF"];
$dirfonts = str_replace("derivemath.php","fonts",$dirfonts);

//learnsquare host path
//edit by Watee wichiennit 03/01/2008
$learnsquare_host = "http://".$_SERVER["SERVER_ADDR"];
...
$dirimg = $_SERVER["DOCUMENT_ROOT"].'/'.$_cid1;
$dirimgrel = $_cid1;
...
function affiche_math($texte, $taille)
{
	...
	$texte=stripslashes($texte);
	if(isset($fontesmath[$texte])) $font = $dirfonts."/".$fontesmath[$texte].".ttf";
	elseif (preg_match("/[a-zA-Z]/i", $texte)) $font = $dirfonts."/cmmi10.ttf";
...
function detectimg($n)
{
	...
	{
		$info=pathinfo($fi);
		if ($fi!="." && $fi!=".." && $info["extension"]=="png" && preg_match("/^math/i",$fi))
		{
