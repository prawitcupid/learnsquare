...
if ($handle = opendir($path.'/backup/backup/system')) {
...
$copy_from_dir = $path.'/backup/backup/system/'.$file;
$copy_to_dir = $path;
	...
	while (false!==($item=readdir($handle)))
	   if (!in_array($item,$exceptions))
	   {
           ...
           if (is_dir($from))
           {
			   ...
			   if($parts["basename"]!="Upgrade" && $parts["basename"]!="courses" && $parts["basename"]!="backup")   
			   {
					if (@mkdir($to))
					{
						chmod($to,$chmod);
