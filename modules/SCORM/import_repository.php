...
switch ($op) {
	case "import_course" :	importCourse($vars);break;
	default :	
													include 'header.php';
													
													/** Navigator **/
													$menus= array(_ADMINMENU,_SCORMADMIN,_SCORMIMPORT);
													$links=array('index.php?mod=Admin','index.php?mod=SCORM&amp;file=admin','index.php?mod=SCORM&amp;file=import');
...
function importForm() {
		...
		<TR>
			...
			<INPUT TYPE="hidden" NAME="file" VALUE="import_repository">
			<INPUT TYPE="hidden" NAME="op" VALUE="import_course">
			<INPUT TYPE="hidden" NAME="fileimport" VALUE="<?php echo $link; ?>" >
			<TABLE>
	<?
			echo ''
	...
	?>
			...
			<TR>
				<TD><?=_PACKAGE?></TD><TD><P><?php echo $title; ?></P></TD>
			</TR>
			<TR>
				<TD>&nbsp;</TD>
...
// Get arguments from argument array
	...
	//echo "link scorm=".$fileimport.'<br>';exit();
...
/* check if ../content/import/ exists */
	...
	if($fileimport){
		...
		$file = fopen($fileimport,"rb");
		if($file){
			//echo "fileimport open";exit();
			$ext = end(explode(".",strtolower(basename($fileimport))));
			$link = fopen($import_path.basename($fileimport), "wb"); // replace "downloads" with whatever directory you wish.
			//echo $link;exit();
			if($link){
				//echo "pass link";exit();
				while(!feof($file)){
					// Write the url file to the directory.
					fwrite($link,fread($file,1024 * 8),1024 * 8);
					// write the file to the new directory at a rate of 8kb/sec. until we reach the end.
				}
			}else{
				echo "error link";exit();
			}
			fclose ($link);
		}else{
			echo "fileimport error";exit();
		}
		fclose ($file);
	}
	...
	$archive = new PclZip($import_path.basename($fileimport));
	//echo $import_path.basename($fileimport).'<br>';
